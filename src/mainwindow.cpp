...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
  construct_all();
  setGeometry({static_cast<int>(prog::env::config::misc_prog_window_init_geometry_x),
               static_cast<int>(prog::env::config::misc_prog_window_init_geometry_y),
               static_cast<int>(prog::env::config::misc_prog_window_init_geometry_width),
               static_cast<int>(prog::env::config::misc_prog_window_init_geometry_height)});
}
...
void MainWindow::connect_signals() {
  ...
  connect(data, SIGNAL(good_csv(std::filesystem::path)), this,
          SLOT(on_database_good_csv(std::filesystem::path)));
}
...
void MainWindow::ensure_config() {
  ...
  for (size_t i = 0; i < prog::env::config::record_tbl_column_width.size(); ++i) {
    if (prog::env::config::record_tbl_column_width[i] == 0) {
      record_tbl->horizontalHeader()->setSectionResizeMode(
...
ErrorType MainWindow::load_database() {
  ...
  if (prog::env::config::misc_use_daily_record_csv) {
    ...
    data_csv_name =
      std::format("data-{:04}-{:02}-{:02}.csv", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday);
  } else {
...
bool MainWindow::query_open_masterduel() {
  ...
  if (!hwnd) {
    ...
    if (clickedbtn == open_md) {
      ...
      j.request_stop();
      j.join();
      if (md_launched.load(std::memory_order_acquire)) {
        return true;
      }
      return false;
...
void MainWindow::on_saveAsBtn_clicked() {
  QString new_csv_filename =
    QFileDialog::getSaveFileName(this, "另存为csv", (prog::env::data_csv_path + "new.csv").c_str(),
                                 "csv(*.csv);;All files(*.*)");
...
void MainWindow::set_qss() {
  ...
  startBtn->setStyleSheet(
    qss.arg(prog::env::config::preprocessed::button_color_start_enabled_background)
  ...
  stopBtn->setStyleSheet(
    qss.arg(prog::env::config::preprocessed::button_color_stop_enabled_background)
  ...
  manual_0Btn->setStyleSheet(
    qss.arg(prog::env::config::preprocessed::button_color_manual0_enabled_background)
  ...
  manual_1Btn->setStyleSheet(
    qss.arg(prog::env::config::preprocessed::button_color_manual1_enabled_background)
