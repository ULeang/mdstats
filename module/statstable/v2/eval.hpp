...
public:
    ...
    using Data = std::variant<int64_t, std::string, std::tuple<pAST, pAST, pAST>, std::tuple<std::string, pAST, pAST>>;
    ...
    // this function just simply transform the boxed type to T, i.e. Value<anytype except char type> -> Value<T>
    template <typename T>
...
namespace PARSER
{
    ...
    std::string keywords[] = {"if", "then", "else", "let", "in"};
    auto Leading_id = Alpha | Char('_');
    auto Following_id = Leading_id | DigitC;
    auto Id = filter(
        std::function<bool(const std::string &)>([](const std::string &s)
                                                 {
    ...
    auto char2Int64 = [](char c)
    {
        return int64_t(c - '0');
    };

    auto Digit64 = fmap<char, int64_t>(char2Int64, DigitC);
    auto mkInt64 = [](Many<int64_t> ints)
    {
        int64_t rec = 0;
        for (int64_t i : ints)
        {
    ...
    auto Natural64 = fmap<Many<int64_t>, int64_t>(mkInt64, many1(Digit64));

    auto Integer64 = Char('-') >> fmap<int64_t, int64_t>([](int64_t i)
                                                         { return int64_t(-1) * i; }, Natural64) |
                     Natural64;
    auto Number = Integer64;

    using pAST = std::shared_ptr<AST>;

    auto Id_AST = fmap<std::string, pAST>([](std::string t)
                                          { return AST::make(AST::id, t); }, Id);
    auto Number_AST = fmap<int64_t, pAST>([](int64_t t)
                                          { return AST::make(AST::number, t); }, Number);

#define DEFPARSER(name, type)                        \
    ParserRet<type> _##name(std::string_view input); \
    auto name = std::function(_##name);

    // we need recursive parser, this is significant, a forward declaration of a function and a function wrapper
    // a forward declaration of a variable would not work, because combinators capture their parameters parser by value
    // at the time we call a combinator, the variable is still a invalid parser result in bad function call
    DEFPARSER(Expression, pAST)

    auto Spaces = TakeWhile([](char c)
                            { return isspace(c); });
    auto Spaces1 = Take1While([](char c)
                              { return isspace(c); });

    auto Parenthetical_Expression = Char('(') >> Spaces >> Expression << Spaces << Char(')');
...
#define DEFBINARY(name, leading, nodetype)                                                             \
    auto name = (leading >>                                                                            \
                ...
                (Spaces >>                                                                             \
                 Expression);                                                                          \
    auto name##_AST = fmap<std::tuple<pAST, pAST>, pAST>(                                              \
    ...
    auto Branch = (Lit("if") >> Spaces1 >> Expression) &
                  (Spaces1 >> Lit("then") >> Spaces1 >> Expression) &
                  (Spaces1 >> Lit("else") >> Spaces1 >> Expression);
    auto Branch_AST = fmap<std::tuple<std::tuple<pAST, pAST>, pAST>, pAST>(
        [](std::tuple<std::tuple<pAST, pAST>, pAST> t)
        {
            return AST::make(AST::branch,
                             std::make_tuple(std::get<0>(std::get<0>(t)),
                                             std::get<1>(std::get<0>(t)),
                                             std::get<1>(t)));
        },
        Branch);

    auto Def = (Lit("let") >> Spaces1 >> Id) &
               (Spaces >> Char('=') >> Spaces >> Expression) &
               (Spaces1 >> Lit("in") >> Spaces1 >> Expression);
    auto Def_AST = fmap<std::tuple<std::tuple<std::string, pAST>, pAST>, pAST>(
        [](std::tuple<std::tuple<std::string, pAST>, pAST> t)
        {
            return AST::make(AST::def,
                             std::make_tuple(std::get<0>(std::get<0>(t)),
                                             std::get<1>(std::get<0>(t)),
                                             std::get<1>(t)));
        },
    ...
    ParserT<char> Eof = [](std::string_view input)
    {
        if (input.empty())
        {
    ...
    ParserRet<pAST>
    _Expression(std::string_view input)
    {
        auto __Expression =
    ...
    auto Type = Char('%') >> (Char('i') | Char('f'));
    auto Type_expression = Type & (Spaces1 >> Expression);
    auto Type_expression_AST = fmap<std::tuple<char, pAST>, pAST>(
        [](std::tuple<char, pAST> t)
        {
            ...
            return p_ast; }, Type_expression);
    auto Single_expression = Spaces >> Type_expression_AST << Spaces << Eof;
}
